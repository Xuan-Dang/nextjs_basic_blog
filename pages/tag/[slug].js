...
function posts({ posts, error, sort, page, limit, count, slug, tag }) {
  const router = useRouter();

  const handleSort = (e) => {
    router.push(
      `/tag/${slug}?page=${page}&limit=${limit}&sort=${e.target.value}`
    );
  };
  const handleChangePage = (newPage) => {
    router.push(`/tag/${slug}?page=${newPage}&limit=${limit}&sort=${sort}`);
  };
  return (
    <Layout>
      {/* Head */}
      <Head>
        <title>{tag?.name ? tag.name : "404"}</title>
        <meta
          name="description"
          content={tag?.description ? tag.description : ""}
        />
      </Head>
      {/* End Head */}
            ...
            <Col>
              ...
              <Breadcrumb>
                ...
                <Breadcrumb.Item active>
                  {tag?.name ? tag.name : "404"}
                </Breadcrumb.Item>
              </Breadcrumb>
              {/* End breadcrumb */}

              {/* Page title */}
              <h1>{tag?.name ? tag.name : "404"}</h1>
              {/* End page title */}
            </Col>
          </Row>
          {/* End header row */}
